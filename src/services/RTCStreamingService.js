...
class RTCStreamingService {
      ...
      this.unsubscribeActiveTests = onSnapshot(activeTestsRef, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          ...
          if (change.type === 'added') {
            ...
            this.connectToStudent(userId, data.testId, data);
          }
   ...
   */
  async connectToStudent(studentId, testId, activeTestData = {}) {
    // Register active student immediately so admin panel can show the user
    this.activeStreams[studentId] = {
      stream: null,
      studentId,
      testId,
      violations: {
        tab: activeTestData.tabViolations || 0,
        noise: activeTestData.noiseViolations || 0
      },
      connected: true
    };
