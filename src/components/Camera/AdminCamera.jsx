...
import React, { useState, useEffect } from 'react';
import { Camera, Users, VideoOff, AlertOctagon, Volume2 } from 'lucide-react';
...
export const AdminCamera = ({ results }) => {
  ...
  const liveStreamEntries = Object.entries(activeStreams).filter(([, streamData]) => {
    return !!streamData?.stream && streamData?.connected !== false;
  });

  const getUserDetails = (userId) => {
    const matchedResult = results?.find((entry) => entry.user === userId || entry.username === userId);
    return {
      user: matchedResult?.user || userId
    };
  };
  ...
  return (
    <div className="admin-camera-container">
      <div className="camera-header">
        ...
        <div className="active-count">
          <Users size={18} />
          <span>{liveStreamEntries.length} Live Users</span>
        </div>
      </div>
      ...
      <div className="camera-grid">
        {liveStreamEntries.length === 0 ? (
          <div className="no-active-users">
            <VideoOff size={48} />
            <p>No live test-takers at the moment</p>
          </div>
        ) : (
          liveStreamEntries.map(([userId, streamData]) => {
